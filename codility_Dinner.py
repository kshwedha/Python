...
def has_same_value(A, B, count):
    ...
    for index in range(len(A)):
        if A[index] == B[index]:
            B = rotate(B)
            count += 1
            return has_same_value(A, B, count)
    return count
...
def solution(A, B):
    ...
    if A:
        if len(A) > 300:
            return
        if all(type(i) == int for i in A) and all(type(i) == int for i in B):
            if len(A) == 1 and A != B:
                return 0
            if all(i == A[0] for i in A) or all(i == B[0] for i in B):
                return -1
            return has_same_value(A, B, 0)
        else:
            return
